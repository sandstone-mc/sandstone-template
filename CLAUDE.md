...
Starter template for creating reusable [Sandstone](https://github.com/sandstone-mc/sandstone) libraries. Libraries export functions and utilities that other Sandstone projects can import.
...
bun dev:test-build  # Build the test datapack (dry run with verbose output)
...
└── *.ts                  # Library modules (MCFunction factories, utilities, etc.)
lib/                      # Built library output (created on build)
├── index.js              # Bundled JavaScript
├── index.d.ts            # TypeScript declarations
└── *.d.ts                # Module declarations
test/                     # Test workspace - a datapack that uses the library
├── src/
│   ├── index.ts          # Test entry point
│   └── *.ts              # Test datapack code that imports from the library
├── sandstone.config.ts   # Test datapack configuration
├── index.test.ts         # Bun snapshot tests
└── package.json          # Links to parent library via `link:sandstone-template`
package.json              # Library package configuration
tsconfig.json             # TypeScript config (declaration generation)
```

**Build outputs:**
- Library: `lib/index.js` + `lib/*.d.ts`
- Test datapack: `test/.sandstone/output/datapack/` (when running tests)

### Test Output Structure

When building the test datapack, output goes to `test/.sandstone/output/`:

```
test/.sandstone/output/
├── datapack/
│   ├── pack.mcmeta
│   └── data/
│       ├── <namespace>/              # Your namespace (configured in test/sandstone.config.ts)
...
- MCFunctions: `test/.sandstone/output/datapack/data/<namespace>/function/<name>.mcfunction`
- The namespace is set in `test/sandstone.config.ts` (defaults to "default")
- Nested function names like `MCFunction('foo/bar', ...)` create `foo/bar.mcfunction`

## Quick Start

Libraries export factory functions that consumers call to generate MCFunctions:

```typescript
// src/display.ts
import { MCFunction, tellraw } from 'sandstone'

// Export a factory function - the MCFunction is created when called
export const displayMessage = () => MCFunction('mylib:display', () => {
  tellraw('@a', { text: 'Hello from library!', color: 'gold' })
}, { runOnLoad: true })
```

```typescript
// src/index.ts - re-export public API
export { displayMessage } from './display.js'
```

**Consumer usage (in a datapack project):**
```typescript
import { displayMessage } from 'your-library-name'

// Call the factory to register the MCFunction
displayMessage()
```

### Library Design Patterns

**Factory functions** - Return MCFunctions that consumers invoke:
```typescript
export const greet = (name: string) => MCFunction(`mylib:greet_${name}`, () => {
  say(`Hello, ${name}!`)
})
```

**Configurable utilities** - Accept options for flexibility:
```typescript
export const timerDisplay = (opts: { interval: number }) => {
  const timer = Objective.create('timer')
  return MCFunction('mylib:timer', () => {
    // Use opts.interval...
  }, { runOnTick: true })
}
```

**Namespace conventions** - Use a consistent namespace prefix:
```typescript
// Good: Prefixed namespace prevents conflicts
MCFunction('mylib:feature/action', () => {...})

// Avoid: Generic names may conflict with consumer code
MCFunction('feature/action', () => {...})
```

## Testing

The `test/` workspace is a mini datapack that imports and uses your library:

1. Library code in `test/src/` imports from `'sandstone-template'` (your library)
2. `bun dev:test-build` runs a dry build and outputs the result
3. `bun test` runs snapshot tests to verify output consistency

**Workflow:**
```bash
# Make changes to src/
bun dev:build         # Rebuild library
bun dev:test-build    # Check test datapack output
bun test              # Update/verify snapshots
```
...
1. Update `name` in `package.json` to your package name (e.g., `@sandstone/my-library`)
2. Update `version` for each release
3. Build with `bun dev:build` (runs automatically via `prepublishOnly`)
4. Publish with `npm publish` or `bun publish`

## Test Configuration

Edit `test/sandstone.config.ts` to change:

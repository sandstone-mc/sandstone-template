...
test.group('Register', () => {
    ...
    test('use package.json dirname as root directory', async ({ fs }) => {
        ...
        await fs.create('bin/server.js', `import * as http from 'http'
       import { join } from 'node:path'
    ...
    test('use package.json restart files', async ({ fs }) => {
      ...
      `);
        ...
        await server.waitForOutput('Server is running');
        await fs.create('.restart-file', '');
        await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:full-reload' && message.path.includes('.restart-file'));
    });
    test('use default reload files when restart files are not defined', async ({ fs }) => {
      ...
      `);
        ...
        await server.waitForOutput('Server is running');
        await fs.create('.env', '');
        await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:full-reload' && message.path.includes('.env'));
    });

...
test.group('Loader', () => {
    test('Works fine', async ({ fs }) => {
        ...
        await fs.create('server.js', `import * as http from 'http'
    ...
    test('send full reload message', async ({ fs, assert }) => {
       ...
       server.listen(3333, () => {
        ...
        await createHandlerFile({ path: 'app.js', response: 'Hello World! Updated' });
        await setTimeout(100);
        const result = await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:full-reload' && message.path === join(fs.basePath, 'app.js'));
    ...
    test('even add import.meta.hot to ignored files', async ({ fs, assert }) => {
        await fakeInstall(fs.basePath);
        await fs.createJson('package.json', { type: 'module' });
        await fs.create('config/test.js', `
       if (import.meta.hot) {
    ...
    `);
       ...
       server.listen(3333, () => {
        ...
        await server.waitForOutput('Server is running');
        await setTimeout(100);
        const result = await pEvent(server.child, 'message', (message) => message?.type === 'ok');
        assert.isDefined(result);
    });
    test('send invalidated message when file is invalidated', async ({ fs, assert }) => {
       ...
       server.listen(3333, () => {
        ...
        const result = await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:invalidated' &&
    ...
    test('full reload when a `restart` file changes', async ({ fs, assert }) => {
       ...
       server.listen(3333, () => {
        ...
        const result = await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:full-reload');
        assert.isDefined(result);
    }).disableTimeout();
    test('full reload if file should be reloadable but is not dynamically imported', async ({ fs, assert, }) => {
        await fakeInstall(fs.basePath);
       ...
       server.listen(3333, () => {
        ...
        await createHandlerFile({ path: 'app.js', response: 'Hello World! Updated' });
        await setTimeout(100);
        const result = await pEvent(server.child, 'message', (message) => message?.type === 'hot-hook:full-reload' && message.shouldBeReloadable === true);
        assert.isDefined(result);
    }).disableTimeout();
    test('send shouldBeReloadable if parent boundary is not dynamically importd', async ({ fs, assert, }) => {
        await fakeInstall(fs.basePath);
        await fs.createJson('package.json', { type: 'module', hotHook: { boundaries: ['./app.js'] } });
        await fs.create('server.js', `import * as http from 'http'
       import { hot } from 'hot-hook'
      ...
      export default function(request, response) {
        ...
        await setTimeout(100);
        const result = await pEvent(server.child, 'message', (message) => {
            console.log(message);
            return message?.type === 'hot-hook:full-reload' && message.shouldBeReloadable === true;
        });
        assert.isDefined(result);
    }).disableTimeout();
    test('throw error if file should be reloadable but is not dynamically imported and flag is set', async ({ fs, assert, }) => {
        await fakeInstall(fs.basePath);

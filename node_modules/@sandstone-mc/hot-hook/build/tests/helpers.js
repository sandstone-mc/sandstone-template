...
export async function fakeInstall(destination) {
    const { name: packageName, bin = {} } = await fs.readJson(path.resolve(projectRoot, 'package.json'));
    await fs.ensureSymlink(projectRoot, path.resolve(destination, 'node_modules', packageName));
    if (typeof bin === 'string') {
        const binPath = bin;
        const binName = packageName;
        await fs.ensureSymlink(path.resolve(projectRoot, binPath), path.resolve(destination, 'node_modules', '.bin', binName));
    }
    else {
...
export async function createHandlerFile(options) {
    const activeTest = getActiveTest();
    if (!activeTest)
        throw new Error('No active test');
    const { path: handlerPath, response } = options;
    await activeTest.context.fs.create(handlerPath, `export default function(request, response) {
      response.writeHead(200, {'Content-Type': 'text/plain'})
      response.end('${response}')
    }`);
}
export function runProcess(scriptPath, options) {
    const activeTest = getActiveTest();
    if (!activeTest) {
    ...
    return {
        child,
        async waitForOutput(output, timeout = 10_000) {

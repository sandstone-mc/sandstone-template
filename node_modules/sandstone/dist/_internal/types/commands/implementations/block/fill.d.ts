import type { Coordinates, RootNBT, SymbolBlock, SymbolMcdocBlockStates } from '../../../arguments/index.js';
import type { Macroable, TagClass } from '../../../core/index.js';
import { CommandNode } from '../../../core/nodes.js';
import { CommandArguments, type FinalCommandOutput } from '../../helpers.js';
import { type BlockEntity, type BlockStatic, type ParseBlockState } from './setblock.js';
import type { AllowConst } from '../../../utils.js';
export declare class FillCommandNode extends CommandNode {
    command: "fill";
}
export declare class FillCommand<MACRO extends boolean> extends CommandArguments {
    protected NodeType: typeof FillCommandNode;
    /**
     ...
     * ```
     */
    fill<BLOCK extends Macroable<BlockEntity, MACRO>>(from: Macroable<Coordinates<MACRO>, MACRO>, to: Macroable<Coordinates<MACRO>, MACRO>, block: BLOCK, state?: Macroable<BLOCK extends keyof SymbolMcdocBlockStates ? ParseBlockState<NonNullable<SymbolMcdocBlockStates[BLOCK]>> : Record<string, string | boolean | number>, MACRO>, nbt?: Macroable<BLOCK extends keyof SymbolBlock ? NonNullable<AllowConst<SymbolBlock[Extract<BLOCK, keyof SymbolBlock>]>> : AllowConst<RootNBT>, MACRO>): FillArgumentsCommand<MACRO>;
}
export declare class FillArgumentsCommand<MACRO extends boolean> extends CommandArguments {
    /**
     ...
     * ```
     */
    replace(filter: Macroable<TagClass<'block'>, MACRO>, filterState?: Macroable<SymbolMcdocBlockStates<'%fallback'>, MACRO>, filterNbt?: Macroable<SymbolBlock<'%fallback'>, MACRO>): FillReplaceFilterCommand;
    /**
     ...
     * ```
     */
    replace<FILTER extends Macroable<BlockStatic, MACRO>>(filter: FILTER, filterState?: Macroable<FILTER extends keyof SymbolMcdocBlockStates ? ParseBlockState<NonNullable<SymbolMcdocBlockStates[FILTER]>> : Record<string, string | boolean | number>, MACRO>): FillReplaceFilterCommand;
    /**
     ...
     * ```
     */
    replace<FILTER extends Macroable<BlockEntity, MACRO>>(filter: FILTER, filterState?: Macroable<FILTER extends keyof SymbolMcdocBlockStates ? ParseBlockState<NonNullable<SymbolMcdocBlockStates[FILTER]>> : Record<string, string | boolean | number>, MACRO>, filterNbt?: Macroable<FILTER extends keyof SymbolBlock ? NonNullable<SymbolBlock[FILTER]> : SymbolBlock<'%fallback'>, MACRO>): FillReplaceFilterCommand;
}
/**
 * Fill modes available after `replace(filter)`.
 * This class is executable on its own (filter with default behavior).
 */
export declare class FillReplaceFilterCommand extends CommandArguments {
    /**

...
export type StringSmallerThan4<T extends string> = T extends `${infer _A}${infer _B}${infer _C}${infer _D}${infer _E}` ? ['ERROR: Pattern row must be 3 characters or smaller.', never] : T;
...
export type CharacterOfStringsArray<T extends readonly (string | undefined)[]> = T extends readonly [infer A, ...infer B extends readonly (string | undefined)[]] ? (A extends string ? CharacterOfString<A> : never) | CharacterOfStringsArray<B> : never;
...
import type { CraftingShaped, Recipe } from './generated/data/recipe.js';
/**
...
} & CraftingShaped<P1, P2, P3>) | Exclude<Recipe, {
    type: 'crafting_shaped' | 'minecraft:crafting_shaped';
}>;
//# sourceMappingURL=shapedCrafting.d.ts.map
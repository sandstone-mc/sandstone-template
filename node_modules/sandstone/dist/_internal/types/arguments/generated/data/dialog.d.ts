import type { ClickAction } from './dialog/action.js';
import type { DialogBody } from './dialog/body.js';
import type { InputControl } from './dialog/input.js';
import type { SymbolMcdocDialogAfterAction } from '../dispatcher.js';
import type { Registry } from '../registry.js';
import type { Text } from '../util/text.js';
import type { RootNBT } from '../../nbt.js';
import type { DialogClass, NBTInt, NBTList, TagClass } from '../../../../index.js';
...
export type DialogBase = NonNullable<({
    [S in Extract<Extract<AfterAction, string>, string>]?: ({
         ...
         */
        after_action?: S;
    } & (S extends undefined ? SymbolMcdocDialogAfterAction<'%none'> : (S extends keyof SymbolMcdocDialogAfterAction ? SymbolMcdocDialogAfterAction[S] : RootNBT)));
}[Extract<AfterAction, string>])>;
export type DialogListRef = (Dialog | Array<Dialog> | (Registry['minecraft:dialog'] | `#${Registry['minecraft:tag/dialog']}` | TagClass<'dialog'> | DialogClass) | Array<(Registry['minecraft:dialog'] | DialogClass)>);
...
export type SymbolDialog<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? DialogDispatcherMap : CASE extends 'keys' ? DialogKeys : CASE extends '%fallback' ? DialogFallback : never;
export {};

import type { MinMaxBounds } from '../util.js';
import type { SymbolBlock, SymbolEntity, SymbolMcdocBlockStates, SymbolStatisticType } from '../../dispatcher.js';
import type { Registry } from '../../registry.js';
import type { EquipmentSlot } from '../../util/slot.js';
import type { DataComponentExactPredicate, DataComponentPredicate } from '../../world/component.js';
import type { AxolotlVariant, FoxType, HorseVariant, LlamaVariant, MooshroomType, ParrotVariant, RabbitVariant, TropicalFishPattern } from '../../world/component/entity.js';
import type { BoatType } from '../../world/entity/boat.js';
import type { ENTITY_SLOTS } from '../../../index.js';
import type { RootNBT } from '../../../nbt.js';
import type { EnchantmentClass, LabelClass, LiteralUnion, NBTClass, NBTDouble, NBTFloat, NBTInt, TagClass, VariantClass } from '../../../../../index.js';
...
export type CatPredicate = {
    variant: ((Registry['minecraft:cat_variant'] | `#${string}:${string}` | TagClass<'cat_variant'> | VariantClass<'cat'>) | Array<(Registry['minecraft:cat_variant'] | VariantClass<'cat'>)>);
};
...
export type EnchantmentPredicate = {
    enchantments?: ((Registry['minecraft:enchantment'] | `#${Registry['minecraft:tag/enchantment']}` | TagClass<'enchantment'> | EnchantmentClass) | Array<(Registry['minecraft:enchantment'] | EnchantmentClass)>);
...
export type EntitySubPredicate = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:entity_sub_predicate_type'], string>, string>]?: ({
        type: S;
    } & (S extends keyof SymbolEntitySubPredicate ? SymbolEntitySubPredicate[S] : RootNBT));
}[Extract<Registry['minecraft:entity_sub_predicate_type'], string>])>;
export type EntitySubPredicateMap = ({
    [Key in Extract<Registry['minecraft:entity_sub_predicate_type'], string>]?: (Key extends keyof SymbolEntitySubPredicate ? SymbolEntitySubPredicate[Key] : RootNBT);
});
...
export type EntityTypePredicate = ((Registry['minecraft:entity_type'] | `#${Registry['minecraft:tag/entity_type']}` | TagClass<'entity_type'>) | Array<Registry['minecraft:entity_type']>);
export type FishingHookPredicate = {
    in_open_water?: boolean;
};
export type FluidPredicate = {
    fluids?: ((Registry['minecraft:fluid'] | `#${Registry['minecraft:tag/fluid']}` | TagClass<'fluid'>) | Array<Registry['minecraft:fluid']>);
    state?: ({
...
export type FrogPredicate = {
    variant: ((Registry['minecraft:frog_variant'] | `#${string}:${string}` | TagClass<'frog_variant'> | VariantClass<'frog'>) | Array<(Registry['minecraft:frog_variant'] | VariantClass<'frog'>)>);
};
...
export type LocationPredicate = {
    ...
    biomes?: ((Registry['minecraft:worldgen/biome'] | `#${Registry['minecraft:tag/worldgen/biome']}` | TagClass<'worldgen/biome'>) | Array<Registry['minecraft:worldgen/biome']>);
    structures?: ((Registry['minecraft:worldgen/structure'] | `#${Registry['minecraft:tag/worldgen/structure']}` | TagClass<'worldgen/structure'>) | Array<Registry['minecraft:worldgen/structure']>);
...
export type OldEntityPredicate = NonNullable<({
...
}[Extract<EntityTypePredicate, string>])>;
export type PaintingPredicate = {
    variant: ((Registry['minecraft:painting_variant'] | `#${Registry['minecraft:tag/painting_variant']}` | TagClass<'painting_variant'> | VariantClass<'painting'>) | Array<(Registry['minecraft:painting_variant'] | VariantClass<'painting'>)>);
};
...
export type SpecificType = ('any' | 'axolotl' | 'boat' | 'cat' | 'fishing_hook' | 'fox' | 'frog' | 'horse' | 'lightning' | 'llama' | 'mooshroom' | 'painting' | 'parrot' | 'player' | 'rabbit' | 'slime' | 'tropical_fish' | 'villager');
export type StatisticPredicate = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:stat_type'], string>, string>]?: {
        type: S;
        stat: (S extends keyof SymbolStatisticType ? SymbolStatisticType[S] : SymbolStatisticType<'%unknown'>);
        value: MinMaxBounds<NBTInt>;
    };
}[Extract<Registry['minecraft:stat_type'], string>])>;
...
export type WolfPredicate = {
    variant: ((Registry['minecraft:wolf_variant'] | `#${string}:${string}` | TagClass<'wolf_variant'> | VariantClass<'wolf'>) | Array<(Registry['minecraft:wolf_variant'] | VariantClass<'wolf'>)>);
};
...
export type SymbolEntitySubPredicate<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? EntitySubPredicateDispatcherMap : CASE extends 'keys' ? EntitySubPredicateKeys : CASE extends '%fallback' ? EntitySubPredicateFallback : never;
export {};

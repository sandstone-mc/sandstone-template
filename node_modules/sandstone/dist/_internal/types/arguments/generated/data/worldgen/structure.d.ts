import type { MobCategory, SpawnerData } from './biome.js';
import type { DecorationStep, HeightmapType, HeightProvider } from '../worldgen.js';
import type { Registry } from '../../registry.js';
import type { FlatWeightedList, NonEmptyWeightedList } from '../../util.js';
import type { NBTObject, RootNBT } from '../../../nbt.js';
import type { NBTFloat, NBTInt, TagClass } from '../../../../../index.js';
...
export type Jigsaw<S = undefined> = ({
    ...
    max_distance_from_center: (S extends undefined ? SymbolJigsawMaxDistanceFromCenter<'%none'> : (S extends keyof SymbolJigsawMaxDistanceFromCenter ? SymbolJigsawMaxDistanceFromCenter[S] : SymbolJigsawMaxDistanceFromCenter<'%unknown'>));
...
export type Structure = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:worldgen/structure_type'], string>, string>]?: ({
        type: S;
        biomes: (Array<Registry['minecraft:worldgen/biome']> | (Registry['minecraft:worldgen/biome'] | `#${Registry['minecraft:tag/worldgen/biome']}` | TagClass<'worldgen/biome'>));
        /**
...
type JigsawMaxDistanceFromCenterFallback = (JigsawMaxDistanceFromCenterBeardBox | JigsawMaxDistanceFromCenterBeardThin | JigsawMaxDistanceFromCenterBury | JigsawMaxDistanceFromCenterFallbackType);
...
export type SymbolJigsawMaxDistanceFromCenter<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? JigsawMaxDistanceFromCenterDispatcherMap : CASE extends 'keys' ? JigsawMaxDistanceFromCenterKeys : CASE extends '%fallback' ? JigsawMaxDistanceFromCenterFallback : CASE extends '%none' ? JigsawMaxDistanceFromCenterNoneType : CASE extends '%unknown' ? JigsawMaxDistanceFromCenterFallbackType : never;
...
type StructureConfigFallback = (StructureConfigBastionRemnant | StructureConfigBuriedTreasure | StructureConfigDesertPyramid | StructureConfigEndCity | StructureConfigFortress | StructureConfigIgloo | StructureConfigJigsaw | StructureConfigJungleTemple | StructureConfigMineshaft | StructureConfigNetherFossil | StructureConfigOceanMonument | StructureConfigOceanRuin | StructureConfigPillagerOutpost | StructureConfigRuinedPortal | StructureConfigShipwreck | StructureConfigStronghold | StructureConfigSwampHut | StructureConfigVillage | StructureConfigWoodlandMansion);
...
export type SymbolStructureConfig<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? StructureConfigDispatcherMap : CASE extends 'keys' ? StructureConfigKeys : CASE extends '%fallback' ? StructureConfigFallback : never;
...
type WorldgenPoolAliasBindingFallback = (WorldgenPoolAliasBindingDirect | WorldgenPoolAliasBindingRandom | WorldgenPoolAliasBindingRandomGroup);
...
export type SymbolWorldgenPoolAliasBinding<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? WorldgenPoolAliasBindingDispatcherMap : CASE extends 'keys' ? WorldgenPoolAliasBindingKeys : CASE extends '%fallback' ? WorldgenPoolAliasBindingFallback : never;
export {};

...
import type { NonEmptyWeightedList } from '../util.js';
import type { NBTObject, RootNBT } from '../../nbt.js';
import type { NBTFloat, NBTInt } from '../../../../index.js';
...
export type DecorationStep = ('raw_generation' | 'lakes' | 'local_modifications' | 'underground_structures' | 'surface_structures' | 'strongholds' | 'underground_ores' | 'underground_decoration' | 'fluid_springs' | 'vegetal_decoration' | 'top_layer_modification');
...
export type HeightmapType = ('MOTION_BLOCKING' | 'MOTION_BLOCKING_NO_LEAVES' | 'OCEAN_FLOOR' | 'OCEAN_FLOOR_WG' | 'WORLD_SURFACE' | 'WORLD_SURFACE_WG');
...
type FloatProviderFallback<T extends NBTObject> = (FloatProviderClampedNormal<T> | FloatProviderConstant<T> | FloatProviderTrapezoid<T> | FloatProviderUniform<T>);
...
export type SymbolFloatProvider<T extends NBTObject, CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? FloatProviderDispatcherMap<T> : CASE extends 'keys' ? FloatProviderKeys : CASE extends '%fallback' ? FloatProviderFallback<T> : never;
...
type HeightProviderFallback = (HeightProviderBiasedToBottom | HeightProviderConstant | HeightProviderTrapezoid | HeightProviderUniform | HeightProviderVeryBiasedToBottom | HeightProviderWeightedList);
...
export type SymbolHeightProvider<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? HeightProviderDispatcherMap : CASE extends 'keys' ? HeightProviderKeys : CASE extends '%fallback' ? HeightProviderFallback : never;
...
type IntProviderFallback<T extends NBTObject> = (IntProviderBiasedToBottom<T> | IntProviderClamped<T> | IntProviderClampedNormal<T> | IntProviderConstant<T> | IntProviderTrapezoid<T> | IntProviderUniform<T> | IntProviderWeightedList<T>);
...
export type SymbolIntProvider<T extends NBTObject, CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? IntProviderDispatcherMap<T> : CASE extends 'keys' ? IntProviderKeys : CASE extends '%fallback' ? IntProviderFallback<T> : never;
export {};

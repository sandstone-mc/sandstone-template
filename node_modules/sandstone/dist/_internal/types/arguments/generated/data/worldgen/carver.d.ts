import type { FloatProvider, HeightProvider, VerticalAnchor } from '../worldgen.js';
...
export type CarverConfigBase = ({
    ...
    replaceable?: (Array<Registry['minecraft:block']> | (Registry['minecraft:block'] | `#${Registry['minecraft:tag/block']}` | TagClass<'block'>));
} & {
    y: HeightProvider;
...
export type CarverListRef = (ConfiguredCarver | Array<ConfiguredCarver> | (Registry['minecraft:worldgen/configured_carver'] | `#${string}:${string}` | TagClass<'worldgen/configured_carver'>) | Array<Registry['minecraft:worldgen/configured_carver']>);
...
export type SymbolCarverConfig<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? CarverConfigDispatcherMap : CASE extends 'keys' ? CarverConfigKeys : CASE extends '%fallback' ? CarverConfigFallback : never;
export {};
//# sourceMappingURL=carver.d.ts.map
import type { HeightmapType, IntProvider } from '../worldgen.js';
import type { SymbolBlock } from '../../dispatcher.js';
import type { Registry } from '../../registry.js';
import type { BlockState } from '../../util/block_state.js';
import type { RootNBT } from '../../../nbt.js';
import type { LootTableClass, NBTFloat, NBTInt, TagClass } from '../../../../../index.js';
...
export type BlockRot = {
    ...
    rottable_blocks?: (Array<Registry['minecraft:block']> | (Registry['minecraft:block'] | `#${Registry['minecraft:tag/block']}` | TagClass<'block'>));
};
...
export type ProtectedBlocks = {
    value: ((`#${Registry['minecraft:tag/block']}` | TagClass<'block'>) | (Registry['minecraft:block'] | `#${Registry['minecraft:tag/block']}` | TagClass<'block'>) | Array<Registry['minecraft:block']>);
};
...
export type SymbolPosRuleTest<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? PosRuleTestDispatcherMap : CASE extends 'keys' ? PosRuleTestKeys : CASE extends '%fallback' ? PosRuleTestFallback : never;
...
type RuleBlockEntityModifierFallback = (RuleBlockEntityModifierAppendLoot | RuleBlockEntityModifierAppendStatic | RuleBlockEntityModifierClear | RuleBlockEntityModifierPassthrough);
...
export type SymbolRuleBlockEntityModifier<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? RuleBlockEntityModifierDispatcherMap : CASE extends 'keys' ? RuleBlockEntityModifierKeys : CASE extends '%fallback' ? RuleBlockEntityModifierFallback : never;
...
type RuleTestFallback = (RuleTestBlockMatch | RuleTestBlockstateMatch | RuleTestRandomBlockMatch | RuleTestRandomBlockstateMatch | RuleTestTagMatch);
...
export type SymbolRuleTest<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? RuleTestDispatcherMap : CASE extends 'keys' ? RuleTestKeys : CASE extends '%fallback' ? RuleTestFallback : never;
...
type TemplateProcessorFallback = (TemplateProcessorBlockAge | TemplateProcessorBlockIgnore | TemplateProcessorBlockRot | TemplateProcessorCapped | TemplateProcessorGravity | TemplateProcessorProtectedBlocks | TemplateProcessorRule);
...
export type SymbolTemplateProcessor<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? TemplateProcessorDispatcherMap : CASE extends 'keys' ? TemplateProcessorKeys : CASE extends '%fallback' ? TemplateProcessorFallback : never;
export {};
//# sourceMappingURL=processor_list.d.ts.map
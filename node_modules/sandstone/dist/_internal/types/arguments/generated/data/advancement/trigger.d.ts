import type { DamagePredicate, DamageSourcePredicate, DistancePredicate, EntityEffectsPredicate, EntityPredicate, ItemPredicate, LocationPredicate } from './predicate.js';
import type { LootCondition } from '../loot/condition.js';
import type { MinMaxBounds } from '../util.js';
import type { SymbolMcdocBlockStates } from '../../dispatcher.js';
import type { Registry } from '../../registry.js';
import type { NBTObject } from '../../../nbt.js';
import type { LootTableClass, NBTFloat, NBTInt, NBTList, RecipeClass } from '../../../../../index.js';
...
export type EnterBlock = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:block'], string>, string>]?: (TriggerBase & {
        block?: S;
        state?: (S extends undefined ? SymbolMcdocBlockStates<'%none'> : (S extends keyof SymbolMcdocBlockStates ? SymbolMcdocBlockStates[S] : SymbolMcdocBlockStates<'%unknown'>));
    });
...
type TriggerFallback = (TriggerAllayDropItemOnBlock | TriggerAnyBlockUse | TriggerAvoidVibration | TriggerBeeNestDestroyed | TriggerBredAnimals | TriggerBrewedPotion | TriggerChangedDimension | TriggerChanneledLightning | TriggerConstructBeacon | TriggerConsumeItem | TriggerCrafterRecipeCrafted | TriggerCuredZombieVillager | TriggerDefaultBlockUse | TriggerEffectsChanged | TriggerEnchantedItem | TriggerEnterBlock | TriggerEntityHurtPlayer | TriggerEntityKilledPlayer | TriggerFallAfterExplosion | TriggerFallFromHeight | TriggerFilledBucket | TriggerFishingRodHooked | TriggerHeroOfTheVillage | TriggerImpossible | TriggerInventoryChanged | TriggerItemDurabilityChanged | TriggerItemUsedOnBlock | TriggerKillMobNearSculkCatalyst | TriggerKilledByArrow | TriggerLevitation | TriggerLightningStrike | TriggerLocation | TriggerNetherTravel | TriggerPlacedBlock | TriggerPlayerGeneratesContainerLoot | TriggerPlayerHurtEntity | TriggerPlayerInteractedWithEntity | TriggerPlayerKilledEntity | TriggerPlayerShearedEquipment | TriggerRecipeCrafted | TriggerRecipeUnlocked | TriggerRideEntityInLava | TriggerShotCrossbow | TriggerSleptInBed | TriggerSlideDownBlock | TriggerSpearMobs | TriggerStartedRiding | TriggerSummonedEntity | TriggerTameAnimal | TriggerTargetHit | TriggerThrownItemPickedUpByEntity | TriggerThrownItemPickedUpByPlayer | TriggerTick | TriggerUsedEnderEye | TriggerUsedTotem | TriggerUsingItem | TriggerVillagerTrade | TriggerVoluntaryExile);
...
export type SymbolTrigger<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? TriggerDispatcherMap : CASE extends 'keys' ? TriggerKeys : CASE extends '%fallback' ? TriggerFallback : never;
export {};

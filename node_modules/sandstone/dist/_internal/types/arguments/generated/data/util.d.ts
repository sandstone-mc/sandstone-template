import type { BlockEntityTarget, EntityTarget } from './loot.js';
import type { NumberProviderRef } from './number_provider.js';
import type { Registry } from '../registry.js';
import type { NBTObject, RootNBT } from '../../nbt.js';
import type { SingleEntityArgument } from '../../index.js';
...
export type NbtProviderSource = ('this' | 'killer' | 'attacker' | 'direct_killer' | 'direct_attacker' | 'killer_player' | 'attacking_player' | 'block_entity');
export type RandomIntGenerator = (NBTInt | ({
    [S in Extract<Extract<RandomIntGeneratorType, string>, string>]?: ({
         ...
         */
        type?: S;
    } & (S extends undefined ? SymbolRandomIntGenerator<'%none'> : (S extends keyof SymbolRandomIntGenerator ? SymbolRandomIntGenerator[S] : RootNBT)));
...
export type SymbolNbtProvider<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? NbtProviderDispatcherMap : CASE extends 'keys' ? NbtProviderKeys : CASE extends '%fallback' ? NbtProviderFallback : never;
...
export type SymbolRandomIntGenerator<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? RandomIntGeneratorDispatcherMap : CASE extends 'keys' ? RandomIntGeneratorKeys : CASE extends '%fallback' ? RandomIntGeneratorFallback : CASE extends '%none' ? RandomIntGeneratorNoneType : never;
...
export type SymbolScoreProvider<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? ScoreProviderDispatcherMap : CASE extends 'keys' ? ScoreProviderKeys : CASE extends '%fallback' ? ScoreProviderFallback : never;
export {};

...
import type { NBTDouble, NBTFloat, NBTInt, NBTList, NBTLong, TagClass } from '../../../../../../index.js';
...
export type MultiNoise = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:worldgen/multi_noise_biome_source_parameter_list'], string>, string>]?: (MultiNoiseBase & {
        preset?: S;
    } & (S extends undefined ? SymbolMultiNoiseBiomeSource<'%none'> : (S extends keyof SymbolMultiNoiseBiomeSource ? SymbolMultiNoiseBiomeSource[S] : SymbolMultiNoiseBiomeSource<'%unknown'>)));
...
export type SymbolBiomeSource<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? BiomeSourceDispatcherMap : CASE extends 'keys' ? BiomeSourceKeys : CASE extends '%fallback' ? BiomeSourceFallback : never;
...
export type SymbolMultiNoiseBiomeSource<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? MultiNoiseBiomeSourceDispatcherMap : CASE extends 'keys' ? MultiNoiseBiomeSourceKeys : CASE extends '%fallback' ? MultiNoiseBiomeSourceFallback : CASE extends '%none' ? MultiNoiseBiomeSourceNoneType : CASE extends '%unknown' ? MultiNoiseBiomeSourceFallbackType : never;
export {};
//# sourceMappingURL=biome_source.d.ts.map
import type { BlockPredicate, ItemPredicate } from '../../data/advancement/predicate.js';
import type { DamageType } from '../../data/damage_type.js';
import type { TrimMaterial, TrimPattern } from '../../data/trim.js';
import type { SoundEventRef } from '../../data/util.js';
import type { Instrument } from '../../data/variants/instrument.js';
import type { BlockStateProvider } from '../../data/worldgen/feature/block_state_provider.js';
import type { SymbolMcdocBlockItemStates, SymbolMcdocBlockStateKeys } from '../../dispatcher.js';
import type { Registry } from '../../registry.js';
import type { AttributeOperation } from '../../util/attribute.js';
import type { Profile } from '../../util/avatar.js';
import type { DyeColor, RGB } from '../../util/color.js';
import type { Direction } from '../../util/direction.js';
import type { MobEffectInstance } from '../../util/effect.js';
import type { Filterable, GlobalPos } from '../../util.js';
import type { EquipmentSlot, EquipmentSlotGroup } from '../../util/slot.js';
import type { Text } from '../../util/text.js';
import type { BannerPatternLayer } from '../block/banner.js';
import type { BlockEntityData } from '../block.js';
import type { ContainerLoot, ContainerSlot, Occupant } from './block.js';
import type { CustomData } from '../component.js';
import type { AxolotlVariant, FoxType, HorseVariant, LlamaVariant, MooshroomType, ParrotVariant, RabbitVariant, SalmonType, TropicalFishPattern } from './entity.js';
import type { AnyEntity } from '../entity.js';
import type { ItemStackTemplate } from '../item.js';
import type { RootNBT } from '../../../nbt.js';
import type { TextureType } from '../../../index.js';
import type { BannerPatternClass, DamageTypeClass, EquipmentClass, InstrumentClass, ItemModelDefinitionClass, JukeboxSongClass, LootTableClass, NBTByte, NBTClass, NBTDouble, NBTFloat, NBTInt, NBTList, NBTLong, RecipeClass, TagClass, TextureClass, TrimMaterialClass, TrimPatternClass, VariantClass } from '../../../../../index.js';
/**
 * *either*
 *
 * List length range: 1..
...
export type blocks_attacks = {
     ...
     */
    bypassed_by?: ((Registry['minecraft:damage_type'] | `#${Registry['minecraft:tag/damage_type']}` | TagClass<'damage_type'> | DamageTypeClass) | Array<(Registry['minecraft:damage_type'] | DamageTypeClass)>);
};
...
export type DamageReduction = {
     ...
     */
    type?: ((Registry['minecraft:damage_type'] | `#${Registry['minecraft:tag/damage_type']}` | TagClass<'damage_type'> | DamageTypeClass) | Array<(Registry['minecraft:damage_type'] | DamageTypeClass)>);
    /**
...
export type DamageResistant = {
     ...
     */
    types: ((Registry['minecraft:damage_type'] | `#${Registry['minecraft:tag/damage_type']}` | TagClass<'damage_type'> | DamageTypeClass) | Array<(Registry['minecraft:damage_type'] | DamageTypeClass)>);
};
...
export type DebugStickState = ({
    [Key in Extract<Registry['minecraft:block'], string>]?: (Key extends keyof SymbolMcdocBlockStateKeys ? SymbolMcdocBlockStateKeys[Key] : SymbolMcdocBlockStateKeys<'%unknown'>);
});
...
export type Equippable = {
     ...
     */
    allowed_entities?: ((Registry['minecraft:entity_type'] | `#${Registry['minecraft:tag/entity_type']}` | TagClass<'entity_type'>) | Array<Registry['minecraft:entity_type']>);
    /**
...
export type ItemUseAnimation = ('none' | 'eat' | 'drink' | 'block' | 'bow' | 'spear' | 'trident' | 'spear' | 'crossbow' | 'spyglass' | 'toot_horn' | 'brush' | 'bundle');
...
export type MobVisibility = {
     ...
     * Entities to match.
     */
    targeting_entity_types: ((Registry['minecraft:entity_type'] | `#${Registry['minecraft:tag/entity_type']}` | TagClass<'entity_type'>) | Array<Registry['minecraft:entity_type']>);
    /**
...
export type RemoveEffectsConsumeEffect = {
    effects: ((Registry['minecraft:mob_effect'] | `#${string}:${string}` | TagClass<'mob_effect'>) | Array<Registry['minecraft:mob_effect']>);
};
export type Repairable = {
    items: ((Registry['minecraft:item'] | `#${Registry['minecraft:tag/item']}` | TagClass<'item'>) | Array<Registry['minecraft:item']>);
};
...
export type ToolRule = {
    blocks: ((Registry['minecraft:block'] | `#${Registry['minecraft:tag/block']}` | TagClass<'block'>) | Array<Registry['minecraft:block']>);
    /**
...
export type SymbolAttributeDisplay<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? AttributeDisplayDispatcherMap : CASE extends 'keys' ? AttributeDisplayKeys : CASE extends '%fallback' ? AttributeDisplayFallback : never;
...
type ConsumeEffectFallback = (ConsumeEffectApplyEffects | ConsumeEffectClearAllEffects | ConsumeEffectPlaySound | ConsumeEffectRemoveEffects | ConsumeEffectTeleportRandomly);
...
export type SymbolConsumeEffect<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? ConsumeEffectDispatcherMap : CASE extends 'keys' ? ConsumeEffectKeys : CASE extends '%fallback' ? ConsumeEffectFallback : never;
...
type DataComponentFallback = (DataComponentAdditionalTradeCost | DataComponentAttackRange | DataComponentAttributeModifiers | DataComponentAxolotlVariant | DataComponentBannerPatterns | DataComponentBaseColor | DataComponentBees | DataComponentBlockEntityData | DataComponentBlockState | DataComponentBlocksAttacks | DataComponentBreakSound | DataComponentBucketEntityData | DataComponentBundleContents | DataComponentCanBreak | DataComponentCanPlaceOn | DataComponentCatCollar | DataComponentCatSoundVariant | DataComponentCatVariant | DataComponentChargedProjectiles | DataComponentChickenSoundVariant | DataComponentChickenVariant | DataComponentConsumable | DataComponentContainer | DataComponentContainerLoot | DataComponentCowSoundVariant | DataComponentCowVariant | DataComponentCreativeSlotLock | DataComponentCustomData | DataComponentCustomModelData | DataComponentCustomName | DataComponentDamage | DataComponentDamageResistant | DataComponentDamageType | DataComponentDeathProtection | DataComponentDebugStickState | DataComponentDye | DataComponentDyedColor | DataComponentEnchantable | DataComponentEnchantmentGlintOverride | DataComponentEnchantments | DataComponentEntityData | DataComponentEquippable | DataComponentFireworkExplosion | DataComponentFireworks | DataComponentFood | DataComponentFoxVariant | DataComponentFrogVariant | DataComponentGlider | DataComponentHorseVariant | DataComponentInstrument | DataComponentIntangibleProjectile | DataComponentItemModel | DataComponentItemName | DataComponentJukeboxPlayable | DataComponentKineticWeapon | DataComponentLlamaVariant | DataComponentLock | DataComponentLodestoneTracker | DataComponentLore | DataComponentMapColor | DataComponentMapDecorations | DataComponentMapId | DataComponentMapPostProcessing | DataComponentMaxDamage | DataComponentMaxStackSize | DataComponentMinimumAttackCharge | DataComponentMooshroomVariant | DataComponentNoteBlockSound | DataComponentOminousBottleAmplifier | DataComponentPaintingVariant | DataComponentParrotVariant | DataComponentPiercingWeapon | DataComponentPigSoundVariant | DataComponentPigVariant | DataComponentPotDecorations | DataComponentPotionContents | DataComponentPotionDurationScale | DataComponentProfile | DataComponentProvidesBannerPatterns | DataComponentProvidesTrimMaterial | DataComponentRabbitVariant | DataComponentRarity | DataComponentRecipes | DataComponentRepairCost | DataComponentRepairable | DataComponentSalmonSize | DataComponentSheepColor | DataComponentShulkerColor | DataComponentStoredEnchantments | DataComponentSulfurCubeContent | DataComponentSuspiciousStewEffects | DataComponentSwingAnimation | DataComponentTool | DataComponentTooltipDisplay | DataComponentTooltipStyle | DataComponentTrim | DataComponentTropicalFishBaseColor | DataComponentTropicalFishPattern | DataComponentTropicalFishPatternColor | DataComponentUnbreakable | DataComponentUseCooldown | DataComponentUseEffects | DataComponentUseRemainder | DataComponentVillagerFood | DataComponentVillagerVariant | DataComponentWeapon | DataComponentWolfCollar | DataComponentWolfSoundVariant | DataComponentWolfVariant | DataComponentWritableBookContent | DataComponentWrittenBookContent | DataComponentZombieNautilusVariant);
...
type DataComponentProvidesBannerPatterns = ((Registry['minecraft:banner_pattern'] | `#${Registry['minecraft:tag/banner_pattern']}` | TagClass<'banner_pattern'> | BannerPatternClass) | Array<(Registry['minecraft:banner_pattern'] | BannerPatternClass)>);
...
type DataComponentZombieNautilusVariant = (Registry['minecraft:zombie_nautilus_variant'] | VariantClass<'zombie_nautilus'>);
export type SymbolDataComponent<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? DataComponentDispatcherMap : CASE extends 'keys' ? DataComponentKeys : CASE extends '%fallback' ? DataComponentFallback : never;
export {};

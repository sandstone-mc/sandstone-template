import type { EnchantmentPredicate, EntityEffectsPredicate, ItemPredicate } from '../../data/advancement/predicate.js';
import type { MinMaxBounds } from '../../data/util.js';
import type { Registry } from '../../registry.js';
import type { AttributeOperation } from '../../util/attribute.js';
import type { EquipmentSlotGroup } from '../../util/slot.js';
import type { Text } from '../../util/text.js';
import type { CustomData } from '../component.js';
import type { FireworkShape } from './item.js';
import type { NBTObject } from '../../../nbt.js';
import type { JukeboxSongClass, NBTDouble, NBTInt, TagClass, TrimMaterialClass, TrimPatternClass } from '../../../../../index.js';
...
export type AttributeModifiersPredicateEntry = {
    attribute?: ((Registry['minecraft:attribute'] | `#${string}:${string}` | TagClass<'attribute'>) | Array<Registry['minecraft:attribute']>);
    id?: `${string}:${string}`;
...
export type JukeboxPlayablePredicate = {
    song?: ((Registry['minecraft:jukebox_song'] | `#${string}:${string}` | TagClass<'jukebox_song'> | JukeboxSongClass) | Array<(Registry['minecraft:jukebox_song'] | JukeboxSongClass)>);
};
export type PotionsPredicate = {
    potions?: PotionTypeMatch;
    effects?: CollectionPredicate<EntityEffectsPredicate>;
};
export type PotionTypeMatch = ((Registry['minecraft:potion'] | `#${Registry['minecraft:tag/potion']}` | TagClass<'potion'>) | Array<Registry['minecraft:potion']>);
export type TrimPredicate = {
    material?: ((Registry['minecraft:trim_material'] | `#${string}:${string}` | TagClass<'trim_material'> | TrimMaterialClass) | Array<(Registry['minecraft:trim_material'] | TrimMaterialClass)>);
    pattern?: ((Registry['minecraft:trim_pattern'] | `#${string}:${string}` | TagClass<'trim_pattern'> | TrimPatternClass) | Array<(Registry['minecraft:trim_pattern'] | TrimPatternClass)>);
};
...
type DataComponentPredicateFallback = (DataComponentPredicateAttributeModifiers | DataComponentPredicateBundleContents | DataComponentPredicateContainer | DataComponentPredicateCustomData | DataComponentPredicateDamage | DataComponentPredicateEnchantments | DataComponentPredicateFireworkExplosion | DataComponentPredicateFireworks | DataComponentPredicateJukeboxPlayable | DataComponentPredicatePotionContents | DataComponentPredicateStoredEnchantments | DataComponentPredicateTrim | DataComponentPredicateVillagerVariant | DataComponentPredicateWritableBookContent | DataComponentPredicateWrittenBookContent | DataComponentPredicateFallbackType);
...
type DataComponentPredicateVillagerVariant = ((Registry['minecraft:villager_type'] | `#${string}:${string}` | TagClass<'villager_type'>) | Array<Registry['minecraft:villager_type']>);
...
export type SymbolDataComponentPredicate<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? DataComponentPredicateDispatcherMap : CASE extends 'keys' ? DataComponentPredicateKeys : CASE extends '%fallback' ? DataComponentPredicateFallback : CASE extends '%unknown' ? DataComponentPredicateFallbackType : never;
export {};

...
import type { GlobalPos } from '../util.js';
import type { NBTInt, NBTIntArray, NBTList, NBTLong } from '../../../../index.js';
...
export type Memories = ({
    [Key in Extract<Registry['minecraft:memory_module_type'], string>]?: (Key extends keyof SymbolMemoryModule ? SymbolMemoryModule[Key] : SymbolMemoryModule<'%unknown'>);
});
...
type MemoryModuleFallback = (MemoryModuleAdmiringDisabled | MemoryModuleAdmiringItem | MemoryModuleAngryAt | MemoryModuleAttackTargetCooldown | MemoryModuleBreezeJumpCooldown | MemoryModuleBreezeJumpInhaling | MemoryModuleBreezeJumpTarget | MemoryModuleBreezeLeavingWater | MemoryModuleBreezeShoot | MemoryModuleBreezeShootCharging | MemoryModuleBreezeShootCooldown | MemoryModuleBreezeShootRecover | MemoryModuleChargeCooldownTicks | MemoryModuleDangerDetectedRecently | MemoryModuleDigCooldown | MemoryModuleGazeCooldownTicks | MemoryModuleGolemDetectedRecently | MemoryModuleHasHuntingCooldown | MemoryModuleHome | MemoryModuleHuntedRecently | MemoryModuleIsEmerging | MemoryModuleIsInWater | MemoryModuleIsPanicking | MemoryModuleIsPregnant | MemoryModuleIsSniffing | MemoryModuleIsTempted | MemoryModuleItemPickupCooldownTicks | MemoryModuleJobSite | MemoryModuleLastSlept | MemoryModuleLastWoken | MemoryModuleLastWorkedAtPoi | MemoryModuleLikedNoteblock | MemoryModuleLikedNoteblockCooldownTicks | MemoryModuleLikedPlayer | MemoryModuleLongJumpCoolingDown | MemoryModuleMeetingPoint | MemoryModulePlayDeadTicks | MemoryModulePotentialJobSite | MemoryModuleRamCooldownTicks | MemoryModuleRecentProjectile | MemoryModuleRoarSoundCooldown | MemoryModuleRoarSoundDelay | MemoryModuleSniffCooldown | MemoryModuleSnifferExploredPositions | MemoryModuleSonicBoomCooldown | MemoryModuleSonicBoomSoundCooldown | MemoryModuleSonicBoomSoundDelay | MemoryModuleTemptationCooldownTicks | MemoryModuleTouchCooldown | MemoryModuleUniversalAnger | MemoryModuleUnreachableTransportBlockPositions | MemoryModuleVibrationCooldown | MemoryModuleVisitedBlockPositions | MemoryModuleFallbackType);
...
export type SymbolMemoryModule<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? MemoryModuleDispatcherMap : CASE extends 'keys' ? MemoryModuleKeys : CASE extends '%fallback' ? MemoryModuleFallback : CASE extends '%unknown' ? MemoryModuleFallbackType : never;
export {};

...
export type Particle = NonNullable<({
    [S in Extract<Extract<Registry['minecraft:particle_type'], string>, string>]?: ({
        type: S;
    } & (S extends undefined ? SymbolParticle<'%none'> : (S extends keyof SymbolParticle ? SymbolParticle[S] : SymbolParticle<'%unknown'>)));
...
type ParticleFallback = (ParticleBlock | ParticleBlockCrumble | ParticleBlockMarker | ParticleDragonBreath | ParticleDust | ParticleDustColorTransition | ParticleDustPillar | ParticleEffect | ParticleEntityEffect | ParticleFallingDust | ParticleFlash | ParticleGeyser | ParticleGeyserBase | ParticleGeyserPlume | ParticleGeyserPoof | ParticleInstantEffect | ParticleItem | ParticleSculkCharge | ParticleShriek | ParticleTintedLeaves | ParticleTrail | ParticleVibration | ParticleFallbackType);
...
export type SymbolParticle<CASE extends 'map' | 'keys' | '%fallback' | '%none' | '%unknown' = 'map'> = CASE extends 'map' ? ParticleDispatcherMap : CASE extends 'keys' ? ParticleKeys : CASE extends '%fallback' ? ParticleFallback : CASE extends '%none' ? ParticleNoneType : CASE extends '%unknown' ? ParticleFallbackType : never;
export {};

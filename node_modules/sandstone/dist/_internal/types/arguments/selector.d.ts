import type { SelectorClass, SelectorPickClass } from '../variables/index.js';
import type { UUIDClass } from '../variables/UUID.js';
...
type MultipleEntitiesSelector<MACRO extends boolean = false> = SelectorClass<MACRO, false, false> | SelectorPickClass<false, false>;
/** Selector classes that target a single entity (any type) */
type SingleEntitySelector<MACRO extends boolean = false> = SelectorClass<MACRO, true, false> | SelectorPickClass<true, false>;
/** Selector classes that target multiple players only */
type MultiplePlayersSelector<MACRO extends boolean = false> = SelectorClass<MACRO, false, true> | SelectorPickClass<false, true>;
/** Selector classes that target a single player only */
type SinglePlayerSelector<MACRO extends boolean = false> = SelectorClass<MACRO, true, true> | SelectorPickClass<true, true>;
export type SelectorArgument<MACRO extends boolean = false, MustBeSingle extends boolean = false, MustBePlayer extends boolean = false> = MustBeSingle extends true ? MustBePlayer extends true ? SinglePlayerStringArgument | SinglePlayerSelector<MACRO> : SingleEntityStringArgument | SingleEntitySelector<MACRO> : MustBePlayer extends true ? MultiplePlayerStringArgument | MultiplePlayersSelector<MACRO> : MultipleEntityStringArgument | MultipleEntitiesSelector<MACRO>;
/**
 ...
 */
export type SinglePlayerArgument<MACRO extends boolean = false> = SinglePlayerStringArgument | SelectorClass<MACRO, true, true> | SelectorPickClass<true, true> | SelectorPickClass<true, false> | SelectorPickClass<true, boolean> | _ShowAlias;
/**
 ...
 */
type PureSelectorPickClass<IsSingle extends boolean, IsPlayer extends boolean> = SelectorPickClass<IsSingle, IsPlayer> & {
    __selectorBrand?: never;
};
/**
